...
def generate_launch_description():
    ...
    pkg_halo_localization = get_package_share_directory("halo_localization")
    pkg_halo_nav = get_package_share_directory("halo_navigation")
    rviz_config = os.path.join(pkg_path, "rviz", "odom.rviz")
    params_file = os.path.join(get_package_share_directory(pkg_path), 'config', 'gazebo.yaml')

    rsp = IncludeLaunchDescription(
            PythonLaunchDescriptionSource([os.path.join(
                get_package_share_directory(pkg_path),'launch','rsp.launch.py'
    ...
    localization_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_halo_localization, "launch", "localization.launch.py")
        ),
        launch_arguments={"use_sim_time": "True"}.items(),
    )
    ekf_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_halo_nav, "launch", "halo_nav_launch.py")
        ),
        launch_arguments={"use_sim_time": "True"}.items(),
    )
